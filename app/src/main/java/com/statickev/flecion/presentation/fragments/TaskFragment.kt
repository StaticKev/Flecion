    ...
    override fun onCreateView(
        ...
        with (binding) {
            ...
            cvPending.setOnClickListener {
                ...
                taskJob = viewLifecycleOwner.lifecycleScope.launch {
                    repeatOnLifecycle(Lifecycle.State.STARTED) {
                        viewModel.pendingTaskState.collect { tasks ->
            ...
            cvOnhold.setOnClickListener {
                ...
                taskJob = viewLifecycleOwner.lifecycleScope.launch {
                    repeatOnLifecycle(Lifecycle.State.STARTED) {
                        viewModel.onHoldTaskState.collect { tasks ->
                            taskAdapter.submitList(tasks)
                        }
                    }
                }
            ...
            cvOngoing.setOnClickListener {
                ...
                taskJob = viewLifecycleOwner.lifecycleScope.launch {
                    repeatOnLifecycle(Lifecycle.State.STARTED) {
                        viewModel.ongoingTaskState.collect { tasks ->
                            taskAdapter.submitList(tasks)
                        }
                    }
                }
            ...
            viewLifecycleOwner.lifecycleScope.launch {
                repeatOnLifecycle(Lifecycle.State.STARTED) {
                    launch {
                        viewModel.pendingTaskState.collect { it ->
                            """Pending (${it.count()})"""
                    ...
                    launch {
                        viewModel.onHoldTaskState.collect { it ->
                            """On Hold (${it.count()})"""
                    ...
                    launch {
                        viewModel.ongoingTaskState.collect { it ->
                            """Ongoing (${it.count()})"""
                        ...
                        viewModel.uiEvent.collect { event ->
                            when (event) {
                                is TaskViewModel.UiEvent.ShowSnackbarOnDelete -> showUndoSnackbar(
                                    ...
                                    viewModel.addTask(event.deletedTask)
                                }
                            }
                        }
                    }
